...
function get_lc_section_completions() {
    ...
    $valueShortcode = '<section class="live-shortcode" lc-helper="shortcode">[lc_get_post post_type="lc_section" slug="' . esc_attr($post->post_name) . '"]</section>';

    return [
        'caption' => 'lc_get_post:section: ' . esc_html($post->post_title),
        'value'   => $valueShortcode, // Encode HTML entities for JavaScript
        'meta'    => $meta
    ];
}, $lc_sections);
